...
class AdminUI(ttk.Frame):
    def __init__(self, master):
        ...
        self.student_tree = ttk.Treeview(self.student_tab, columns=("MSSV", "HOTEN", "Number_of_Attendances"))
        ...
        self.student_tree.heading("Number_of_Attendances", text="Số ngày điểm danh")
        self.btn_frame = tk.Frame(self.student_tab)
        self.add_btn = tk.Button(self.btn_frame, text="Add Student", command=self.add_student)
        self.edit_btn = tk.Button(self.btn_frame, text="Edit Student", command=self.edit_student)
        self.delete_btn = tk.Button(self.btn_frame, text="Delete Student", command=self.delete_student)
    ...
    def hide(self):
        for w in (self, self.notebook, self.student_tab, self.student_tree, self.student_tree, self.btn_frame, self.add_btn, self.edit_btn, self.delete_btn, self.back_btn):
            w.pack_forget()

    def load_students(self):
        self.student_tree.delete(*self.student_tree.get_children())  # Clear existing data
        students = db.get_attendance_data(db.create_connection(config.database_str))
    ...
    def save_student(self):
        ...
        if mssv and name:
            db.insert_student(db.create_connection(config.database_str), mssv, name)
    ...
    def update_student(self, selected_item):
        ...
        if mssv and name:
            db.update_student(db.create_connection(config.database_str), mssv, name)
            self.load_students()  # Reload student list
        else:
            tk.messagebox.showerror("Error", "Please enter both MSSV and Name.")
    ...
    def delete_student(self):
        ...
        if selected_item:
            if tk.messagebox.askyesno("Delete Student", "Are you sure you want to delete this student?"):
                data = self.student_tree.item(selected_item)['values']
                mssv = data[0]
                db.delete_student(db.create_connection(config.database_str), mssv)
                self.load_students()  # Reload student list
        else:
